        ...
        switch (last)
        {
            ...
            case '/':
                ...
                if (last == '#')
                {
                    if (in_dev_state_)
                    {
                        throw s2::parser::syntax_error(loc_, "cannot recurse devblock ('/#')");
                    }
            ...
            case '#':
                ...
                {
                    if (!in_dev_state_)
                        throw s2::parser::syntax_error(loc_, "unmatched devblock end ('#/')");
            ...
            default:
                ...
                throw s2::parser::syntax_error(loc_, utils::string::va("bad token: \'%c\'", last));
        }
    }
}

auto lexer::read_string(char quote, bool localize) -> xsk::gsc::s2::parser::symbol_type
{
    if (localize)
        reader_.advance();
    ...
    if (reader_.state == reader::end)
    {
        throw s2::parser::syntax_error(loc_, utils::string::va("unmatched string start ('%s')", (quote == '"') ? "\"" : "\\'"));
    }
...
auto lexer::read_number(char first) -> xsk::gsc::s2::parser::symbol_type
{
    if (first == '.')
    {
        buffer_.push(first);
