        ...
        switch (last)
        {
            ...
            case '/':
                ...
                if (last == '#')
                {
                    if (in_dev_state_)
                    {
                        throw iw7::parser::syntax_error(loc_, "cannot recurse devblock ('/#')");
                    }
            ...
            case '#':
                ...
                {
                    if (!in_dev_state_)
                        throw iw7::parser::syntax_error(loc_, "unmatched devblock end ('#/')");
            ...
            default:
                ...
                throw iw7::parser::syntax_error(loc_, utils::string::va("bad token: \'%c\'", last));
        }
    }
}

auto lexer::read_string(char quote, bool localize) -> xsk::gsc::iw7::parser::symbol_type
{
    if (localize)
        reader_.advance();
    ...
    if (reader_.state == reader::end)
    {
        throw iw7::parser::syntax_error(loc_, utils::string::va("unmatched string start ('%s')", (quote == '"') ? "\"" : "\\'"));
    }
...
auto lexer::read_number(char first) -> xsk::gsc::iw7::parser::symbol_type
{
    if (first == '.')
    {
        buffer_.push(first);
