        ...
        switch (last)
        {
            ...
            case '/':
                ...
                if (last == '#')
                {
                    if (in_dev_state_)
                    {
                        throw s4::parser::syntax_error(loc_, "cannot recurse devblock ('/#')");
                    }
            ...
            case '#':
                ...
                {
                    if (!in_dev_state_)
                        throw s4::parser::syntax_error(loc_, "unmatched devblock end ('#/')");
            ...
            default:
                ...
                throw s4::parser::syntax_error(loc_, utils::string::va("bad token: \'%c\'", last));
        }
    }
}

auto lexer::read_string(char quote, bool localize) -> xsk::gsc::s4::parser::symbol_type
{
    if (localize)
        reader_.advance();
    ...
    if (reader_.state == reader::end)
    {
        throw s4::parser::syntax_error(loc_, utils::string::va("unmatched string start ('%s')", (quote == '"') ? "\"" : "\\'"));
    }
...
auto lexer::read_number(char first) -> xsk::gsc::s4::parser::symbol_type
{
    if (first == '.')
    {
        buffer_.push(first);
...
auto lexer::read_word(char first) -> xsk::gsc::s4::parser::symbol_type
{
    ...
    while (reader_.state == reader::ok)
    {
        ...
        {
            if (reader_.last_byte == '\\')
                throw s4::parser::syntax_error(loc_, "invalid path '\\\\'");
    ...
    if (path)
    {
        if (buffer_.data[buffer_.length - 1] == '/')
            throw s4::parser::syntax_error(loc_, "invalid path end '\\'");
...
auto lexer::read_dotsize() -> xsk::gsc::s4::parser::symbol_type
{
    ...
    else if (curr == '_' || curr > 64 && curr < 91 || curr > 96 && curr < 123)
    {
        ...
        return s4::parser::make_DOT(loc_);
