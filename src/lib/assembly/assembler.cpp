...
assembler::assembler()
{
	script_ = std::make_unique<xsk::byte_buffer>(0x100000);
	stack_ = std::make_unique<xsk::byte_buffer>(0x100000);
}

void assembler::assemble(std::string& buffer)
{
	XSK_DEBUG("parsing assembly file...");

	std::vector<std::string> assembly = xsk::string::clean_buffer_lines(buffer);
	...
	for (auto& line : assembly)
	{
		...
		else
		{
			...
			if (switchnum)
			{
				...
				XSK_ERROR("invalid instruction inside endswitch \"%s\"!", line.c_str());
				return;
			}
			else
			{
				...
				inst->index = index;
				inst->opcode = get_opcode_id(xsk::string::to_lower(data[0]));
				inst->size = get_opcode_size(inst->opcode);
				inst->data = data;
				inst->parent = func;

				// group switch in one instruction
				if (inst->opcode == opcode::OP_endswitch)
				{
					if (xsk::string::is_number(data[1]))
					{
					...
					{
						XSK_ERROR("endswitch arg is not a number!", line.c_str());
						return;
					}
				}
	...
	XSK_DEBUG("assembly file parse complete.");
	XSK_DEBUG("%d functions staged for assemble.", functions.size());
...
void assembler::assemble_function(std::shared_ptr<function> func)
{
	...
	// function id
	func->id = xsk::string::is_number(func->name) ? std::stoul(func->name) : get_token_id(func->name);
...
void assembler::assemble_instruction(std::shared_ptr<instruction> inst)
{
	...
	case opcode::OP_GetIString:
		...
		stack_->write_string(xsk::string::get_string_literal(inst->data[1]));
		break;
	case opcode::OP_GetAnimation:
		script_->write<std::uint8_t>(static_cast<std::uint8_t>(inst->opcode));
		script_->write<std::uint32_t>(0); // placeholder 4 bytes?
		stack_->write_string(xsk::string::get_string_literal(inst->data[1]));
		stack_->write_string(xsk::string::get_string_literal(inst->data[2]));
		break;
...
void assembler::assemble_end_switch(std::shared_ptr<instruction> inst)
{
	...
	if (xsk::string::is_number(inst->data[1]))
	{
		casenum = std::stol(inst->data[1]);
	}
	else
	{
		XSK_ERROR("invalid endswitch number!");
	}
	...
	for (std::uint16_t i = 0; i < casenum; i++)
	{
		if (inst->data[2 + (3 * i)] == "case")
		{
			...
			stack_->write_string(xsk::string::get_string_literal(inst->data[2 + (3 * i) + 1]));
...
auto assembler::resolve_function(const std::string& name) -> std::uint32_t
{
	...
	XSK_ERROR("Couldn't resolve local function address of '%s'!", name.c_str());
	return 0;
}
...
auto assembler::resolve_label(std::shared_ptr<instruction> inst, const std::string& name) -> std::uint32_t
{
	...
	XSK_ERROR("Couldn't resolve label address of '%s'!", name.c_str());
	return 0;
}
