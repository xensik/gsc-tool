...
auto get_opcode_id(const std::string& name) -> opcode
{
	...
	XSK_ERROR("Couldn't resolve opcode id for name '%s'!", name.c_str());
...
auto get_opcode_name(opcode id) -> std::string
{
	...
	XSK_ERROR("Couldn't resolve opcode name for id '0x%X'!", id);
	return "";
}
...
auto get_opcode_size(opcode id) -> std::size_t
{
	...
	case opcode::OP_GetVector:
		...
		break;
	default:
		XSK_ERROR("Couldn't resolve instruction size for 0x%X!", id);
		return 0;
	}
}
