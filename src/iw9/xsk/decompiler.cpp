...
auto decompiler::output() -> std::vector<std::uint8_t>
{
    ...
    data->write_string("// IW9 GSC SOURCE\n");
...
void decompiler::decompile_instruction(const instruction::ptr& inst)
{
    ...
    switch (static_cast<opcode>(inst->opcode))
    {
        ...
        {
            auto expr = std::make_unique<ast::expr_identifier>(loc, inst->data[0]);
...
void decompiler::process_parameters(const ast::expr_parameters::ptr& params, const block::ptr& blk)
{
    ...
    {
        blk->local_vars.push_back({ entry->value, static_cast<uint8_t>(blk->local_vars_create_count), true });
