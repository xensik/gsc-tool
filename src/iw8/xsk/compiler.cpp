...
auto compiler::parse_buffer(const std::string& file, std::vector<std::uint8_t>& data) -> gsc::program_ptr
{
    ...
    if (iw8_lex_init(&scanner))
        exit(1);

    YY_BUFFER_STATE yybuffer = iw8__scan_buffer(reinterpret_cast<char*>(data.data()), data.size(), scanner);
    ...
    iw8__delete_buffer(yybuffer, scanner);
    iw8_lex_destroy(scanner);
...
{
    std::uint32_t num = params->list.size();

    if(num)
    {
        std::vector<std::string> data;

        data.push_back(utils::string::va("%d", params->list.size()));

        for(const auto& param : params->list)
        {
            initialize_variable(ctx, param);
            data.push_back(variable_create_index(ctx, param));
        }

        emit_opcode(ctx, opcode::OP_FormalParams, data);
...
void compiler::emit_stmt_foreach(const gsc::context_ptr& ctx, const gsc::stmt_foreach_ptr& stmt)
{
    ...
    emit_opcode(ctx, opcode::OP_IsDefined);
	emit_opcode(ctx, opcode::OP_JumpOnFalse, break_loc);
...
void compiler::emit_expr_isdefined(const gsc::context_ptr& ctx, const gsc::expr_arguments_ptr& expr)
{
    emit_expr_arguments(ctx, expr);
    emit_opcode(ctx, opcode::OP_IsDefined);
}

void compiler::emit_expr_istrue(const gsc::context_ptr& ctx, const gsc::expr_arguments_ptr& expr)
{
    emit_expr_arguments(ctx, expr);
    emit_opcode(ctx, opcode::OP_IsTrue);
}

void compiler::emit_expr_bool_not_after_and(const gsc::context_ptr& ctx, const gsc::expr_ptr& expr)
{
    emit_expr(ctx, expr);
    emit_opcode(ctx, opcode::OP_BoolNotAfterAnd);
}
...
void compiler::emit_expr_and(const gsc::context_ptr& ctx, const gsc::expr_and_ptr& expr)
{
    ...
    if(expr->rvalue.as_node->type == gsc::node_t::expr_not)
    {
        emit_expr_bool_not_after_and(ctx, expr->rvalue.as_not->rvalue);
    }
    else
    {
        emit_expr(ctx, expr->rvalue);
...
{
    if(expr->func.as_func->name->value == "isdefined")
    {
        emit_expr_isdefined(ctx, expr->func.as_func->args);
        return;
    }
    else if(expr->func.as_func->name->value == "istrue")
    {
        emit_expr_isdefined(ctx, expr->func.as_func->args);
        return;
    }
