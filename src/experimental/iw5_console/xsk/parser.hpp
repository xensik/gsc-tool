...
#ifndef YY_IW5C_PARSER_HPP_INCLUDED
# define YY_IW5C_PARSER_HPP_INCLUDED
...
#ifdef IW5CSTYPE
# ifdef __GNUC__
#  pragma GCC message "bison: do not #define IW5CSTYPE in C++, use %define api.value.type"
# endif
    typedef IW5CSTYPE value_type;
#else
...
# endif
    ...
    {
      IW5C_ASSERT (yytypeid_);
      IW5C_ASSERT (*yytypeid_ == typeid (T));
      IW5C_ASSERT (sizeof (T) <= size);
      return *yyas_<T> ();
    }
    ...
    {
      IW5C_ASSERT (yytypeid_);
      IW5C_ASSERT (*yytypeid_ == *that.yytypeid_);
...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    struct token
    {
      ...
      {
        IW5CEMPTY = -2,
    IW5CEOF = 0,                   // "end of file"
    IW5Cerror = 1,                 // error
    IW5CUNDEF = 2,                 // "invalid token"
    SH_DEFINE = 3,                 // "#define"
    SH_UNDEF = 4,                  // "#undef"
    SH_IFDEF = 5,                  // "#ifdef"
    SH_IFNDEF = 6,                 // "#ifndef"
...
switch (yykind)
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
...
#endif
      {
        ...
        IW5C_ASSERT (tok == token::IW5CEOF
                   || (token::IW5Cerror <= tok && tok <= token::MOD)
      ...
      {
#if !defined _MSC_VER || defined __clang__
        IW5C_ASSERT ((token::PATH <= tok && tok <= token::INTEGER));
#endif
      }
    };
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_IW5CEOF (location_type l)
      {
        return symbol_type (token::IW5CEOF, std::move (l));
      }
#else
      static
      symbol_type
      make_IW5CEOF (const location_type& l)
      {
        return symbol_type (token::IW5CEOF, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_IW5Cerror (location_type l)
      {
        return symbol_type (token::IW5Cerror, std::move (l));
      }
#else
      static
      symbol_type
      make_IW5Cerror (const location_type& l)
      {
        return symbol_type (token::IW5Cerror, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_IW5CUNDEF (location_type l)
      {
        return symbol_type (token::IW5CUNDEF, std::move (l));
      }
#else
      static
      symbol_type
      make_IW5CUNDEF (const location_type& l)
      {
        return symbol_type (token::IW5CUNDEF, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
...
#endif // !YY_IW5C_PARSER_HPP_INCLUDED
