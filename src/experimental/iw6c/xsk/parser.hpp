...
/**
 ** \file parser.hpp
 ** Define the xsk::gsc::iw6c::parser class.
 */
...
#ifndef YY_IW6C_PARSER_HPP_INCLUDED
# define YY_IW6C_PARSER_HPP_INCLUDED
...
#endif  /* ! defined IW6CDEBUG */

#line 13 "parser.ypp"
namespace xsk { namespace gsc { namespace iw6c {
#line 207 "parser.hpp"
...
#ifdef IW6CSTYPE
# ifdef __GNUC__
#  pragma GCC message "bison: do not #define IW6CSTYPE in C++, use %define api.value.type"
# endif
    typedef IW6CSTYPE value_type;
#else
...
# endif
    ...
    {
      IW6C_ASSERT (yytypeid_);
      IW6C_ASSERT (*yytypeid_ == typeid (T));
      IW6C_ASSERT (sizeof (T) <= size);
      return *yyas_<T> ();
    }
    ...
    {
      IW6C_ASSERT (yytypeid_);
      IW6C_ASSERT (*yytypeid_ == *that.yytypeid_);
...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    struct token
    {
      ...
      {
        IW6CEMPTY = -2,
    IW6CEOF = 0,                   // "end of file"
    IW6Cerror = 1,                 // error
    IW6CUNDEF = 2,                 // "invalid token"
    SH_DEFINE = 3,                 // "#define"
    SH_UNDEF = 4,                  // "#undef"
    SH_IFDEF = 5,                  // "#ifdef"
    SH_IFNDEF = 6,                 // "#ifndef"
...
switch (yykind)
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
...
#endif
      {
        ...
        IW6C_ASSERT (tok == token::IW6CEOF
                   || (token::IW6Cerror <= tok && tok <= token::MOD)
      ...
      {
#if !defined _MSC_VER || defined __clang__
        IW6C_ASSERT ((token::PATH <= tok && tok <= token::INTEGER));
#endif
      }
    };

    /// Build a parser object.
    parser (xsk::gsc::iw6c::lexer& lexer_yyarg, xsk::gsc::ast::program::ptr& ast_yyarg);
    virtual ~parser ();
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_IW6CEOF (location_type l)
      {
        return symbol_type (token::IW6CEOF, std::move (l));
      }
#else
      static
      symbol_type
      make_IW6CEOF (const location_type& l)
      {
        return symbol_type (token::IW6CEOF, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_IW6Cerror (location_type l)
      {
        return symbol_type (token::IW6Cerror, std::move (l));
      }
#else
      static
      symbol_type
      make_IW6Cerror (const location_type& l)
      {
        return symbol_type (token::IW6Cerror, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_IW6CUNDEF (location_type l)
      {
        return symbol_type (token::IW6CUNDEF, std::move (l));
      }
#else
      static
      symbol_type
      make_IW6CUNDEF (const location_type& l)
      {
        return symbol_type (token::IW6CUNDEF, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
...
#endif // !YY_IW6C_PARSER_HPP_INCLUDED
