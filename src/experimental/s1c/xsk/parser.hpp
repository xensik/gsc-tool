...
/**
 ** \file parser.hpp
 ** Define the xsk::gsc::s1c::parser class.
 */
...
#ifndef YY_S1C_PARSER_HPP_INCLUDED
# define YY_S1C_PARSER_HPP_INCLUDED
...
#ifdef S1CSTYPE
# ifdef __GNUC__
#  pragma GCC message "bison: do not #define S1CSTYPE in C++, use %define api.value.type"
# endif
    typedef S1CSTYPE value_type;
#else
...
# endif
    ...
    {
      S1C_ASSERT (yytypeid_);
      S1C_ASSERT (*yytypeid_ == *that.yytypeid_);
...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    struct token
    {
      ...
      {
        S1CEMPTY = -2,
    S1CEOF = 0,                    // "end of file"
    S1Cerror = 1,                  // error
    S1CUNDEF = 2,                  // "invalid token"
    SH_DEFINE = 3,                 // "#define"
    SH_UNDEF = 4,                  // "#undef"
    SH_IFDEF = 5,                  // "#ifdef"
    SH_IFNDEF = 6,                 // "#ifndef"
...
switch (yykind)
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
...
#endif
      {
        ...
        S1C_ASSERT (tok == token::S1CEOF
                   || (token::S1Cerror <= tok && tok <= token::MOD)
      ...
      {
#if !defined _MSC_VER || defined __clang__
        S1C_ASSERT ((token::PATH <= tok && tok <= token::INTEGER));
#endif
      }
    };

    /// Build a parser object.
    parser (xsk::gsc::s1c::lexer& lexer_yyarg, xsk::gsc::ast::program::ptr& ast_yyarg);
    virtual ~parser ();
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_S1CEOF (location_type l)
      {
        return symbol_type (token::S1CEOF, std::move (l));
      }
#else
      static
      symbol_type
      make_S1CEOF (const location_type& l)
      {
        return symbol_type (token::S1CEOF, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_S1Cerror (location_type l)
      {
        return symbol_type (token::S1Cerror, std::move (l));
      }
#else
      static
      symbol_type
      make_S1Cerror (const location_type& l)
      {
        return symbol_type (token::S1Cerror, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_S1CUNDEF (location_type l)
      {
        return symbol_type (token::S1CUNDEF, std::move (l));
      }
#else
      static
      symbol_type
      make_S1CUNDEF (const location_type& l)
      {
        return symbol_type (token::S1CUNDEF, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
...
#endif // !YY_S1C_PARSER_HPP_INCLUDED
