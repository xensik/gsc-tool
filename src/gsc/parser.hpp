...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // expr_add_array
      char dummy6[sizeof (expr_add_array::ptr)];

      // expr_anim
      char dummy7[sizeof (expr_anim::ptr)];

      // expr_animation
      char dummy8[sizeof (expr_animation::ptr)];

      // expr_animtree
      char dummy9[sizeof (expr_animtree::ptr)];

      // expr_arguments
      // expr_arguments_no_empty
      char dummy10[sizeof (expr_arguments::ptr)];

      // expr_array
      char dummy11[sizeof (expr_array::ptr)];

      // expr_call
      char dummy12[sizeof (expr_call::ptr)];

      // expr_complement
      char dummy13[sizeof (expr_complement::ptr)];

      // expr_empty_array
      char dummy14[sizeof (expr_empty_array::ptr)];

      // expr_false
      char dummy15[sizeof (expr_false::ptr)];

      // expr_field
      char dummy16[sizeof (expr_field::ptr)];

      // expr_float
      char dummy17[sizeof (expr_float::ptr)];

      // expr_game
      char dummy18[sizeof (expr_game::ptr)];

      // expr_identifier_nosize
      // expr_identifier
      char dummy19[sizeof (expr_identifier::ptr)];

      // expr_integer
      char dummy20[sizeof (expr_integer::ptr)];

      // expr_isdefined
      char dummy21[sizeof (expr_isdefined::ptr)];

      // expr_istring
      char dummy22[sizeof (expr_istring::ptr)];

      // expr_istrue
      char dummy23[sizeof (expr_istrue::ptr)];

      // expr_level
      char dummy24[sizeof (expr_level::ptr)];

      // expr_method
      char dummy25[sizeof (expr_method::ptr)];

      // expr_negate
      char dummy26[sizeof (expr_negate::ptr)];

      // expr_not
      char dummy27[sizeof (expr_not::ptr)];

      // expr_parameters
      char dummy28[sizeof (expr_parameters::ptr)];

      // expr_paren
      char dummy29[sizeof (expr_paren::ptr)];

      // expr_path
      char dummy30[sizeof (expr_path::ptr)];

      // expr_reference
      char dummy31[sizeof (expr_reference::ptr)];

      // expr_self
      char dummy32[sizeof (expr_self::ptr)];

      // expr_size
      char dummy33[sizeof (expr_size::ptr)];

      // expr_string
      char dummy34[sizeof (expr_string::ptr)];

      // expr_thisthread
      char dummy35[sizeof (expr_thisthread::ptr)];

      // expr_true
      char dummy36[sizeof (expr_true::ptr)];

      // expr_tuple_arguments
      char dummy37[sizeof (expr_tuple::ptr)];

      // expr_undefined
      char dummy38[sizeof (expr_undefined::ptr)];

      // expr_vector
      char dummy39[sizeof (expr_vector::ptr)];

      // include
      char dummy40[sizeof (include::ptr)];

      // program
      char dummy41[sizeof (program::ptr)];
      ...
      // stmt
      // stmt_or_dev
      char dummy43[sizeof (stmt)];

      // stmt_assign
      char dummy44[sizeof (stmt_assign::ptr)];

      // stmt_break
      char dummy45[sizeof (stmt_break::ptr)];

      // stmt_breakpoint
      char dummy46[sizeof (stmt_breakpoint::ptr)];

      // stmt_call
      char dummy47[sizeof (stmt_call::ptr)];
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_decl_function = 115,                   // decl_function
        S_stmt = 116,                            // stmt
        S_stmt_or_dev = 117,                     // stmt_or_dev
        S_stmt_list = 118,                       // stmt_list
        S_stmt_or_dev_list = 119,                // stmt_or_dev_list
        S_stmt_dev = 120,                        // stmt_dev
        S_stmt_comp = 121,                       // stmt_comp
        S_stmt_expr = 122,                       // stmt_expr
        S_stmt_call = 123,                       // stmt_call
        S_stmt_assign = 124,                     // stmt_assign
        S_stmt_endon = 125,                      // stmt_endon
        S_stmt_notify = 126,                     // stmt_notify
        S_stmt_wait = 127,                       // stmt_wait
        S_stmt_waittill = 128,                   // stmt_waittill
        S_stmt_waittillmatch = 129,              // stmt_waittillmatch
        S_stmt_waittillframeend = 130,           // stmt_waittillframeend
        S_stmt_waitframe = 131,                  // stmt_waitframe
        S_stmt_if = 132,                         // stmt_if
        S_stmt_ifelse = 133,                     // stmt_ifelse
        S_stmt_while = 134,                      // stmt_while
        S_stmt_dowhile = 135,                    // stmt_dowhile
        S_stmt_for = 136,                        // stmt_for
        S_stmt_foreach = 137,                    // stmt_foreach
        S_stmt_switch = 138,                     // stmt_switch
        S_stmt_case = 139,                       // stmt_case
        S_stmt_default = 140,                    // stmt_default
        S_stmt_break = 141,                      // stmt_break
        S_stmt_continue = 142,                   // stmt_continue
        S_stmt_return = 143,                     // stmt_return
        S_stmt_breakpoint = 144,                 // stmt_breakpoint
        S_stmt_prof_begin = 145,                 // stmt_prof_begin
        S_stmt_prof_end = 146,                   // stmt_prof_end
        S_expr = 147,                            // expr
        S_expr_or_empty = 148,                   // expr_or_empty
        S_expr_assign = 149,                     // expr_assign
        S_expr_increment = 150,                  // expr_increment
        S_expr_decrement = 151,                  // expr_decrement
        S_expr_ternary = 152,                    // expr_ternary
        S_expr_binary = 153,                     // expr_binary
        S_expr_primitive = 154,                  // expr_primitive
        S_expr_complement = 155,                 // expr_complement
        S_expr_negate = 156,                     // expr_negate
        S_expr_not = 157,                        // expr_not
        S_expr_call = 158,                       // expr_call
        S_expr_method = 159,                     // expr_method
        S_expr_function = 160,                   // expr_function
        S_expr_pointer = 161,                    // expr_pointer
        S_expr_add_array = 162,                  // expr_add_array
        S_expr_parameters = 163,                 // expr_parameters
        S_expr_arguments = 164,                  // expr_arguments
        S_expr_arguments_no_empty = 165,         // expr_arguments_no_empty
        S_expr_isdefined = 166,                  // expr_isdefined
        S_expr_istrue = 167,                     // expr_istrue
        S_expr_reference = 168,                  // expr_reference
        S_expr_tuple = 169,                      // expr_tuple
        S_expr_tuple_arguments = 170,            // expr_tuple_arguments
        S_expr_tuple_types = 171,                // expr_tuple_types
...
switch (yykind)
  ...
  private:
    ...
    enum
    {
      yylast_ = 2452,     ///< Last index in yytable_.
