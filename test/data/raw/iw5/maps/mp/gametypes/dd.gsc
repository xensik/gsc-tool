...
onStartGameType()
{
	...
	//defender spawns added to tdm spawns
	level.spawn_defenders = array_combine( level.spawn_defenders, level.tdmSpawns );
	level.spawn_defenders_a = maps\mp\gametypes\_spawnlogic::getSpawnpointArray( "mp_dd_spawn_defender_a" );

	//IW5 adding defender A spawn close to ent
	...
	//IW5 adding defender B spawn close to ent
	...
	//attacker spawns added to tdm spawns
	level.spawn_attackers = array_combine( level.spawn_defenders, level.tdmSpawns );
	level.spawn_attackers_a = maps\mp\gametypes\_spawnlogic::getSpawnpointArray( "mp_dd_spawn_attacker_a" );

	//IW5 adding attacker A spawn close to ent
	...
	//IW5 adding attacker B spawn close to ent
...
bombHandler( player, destType, playerTeam )	// playerTeam needed because bombHandler is called after a wait so the player could be gone
{
	...
	if ( destType == "explode" )
	{
		...
		if ( !inOvertime() && level.bombExploded < 2 && level.ddTimeToAdd > 0 )
		{
