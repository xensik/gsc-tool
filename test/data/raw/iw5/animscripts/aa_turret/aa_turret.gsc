...
main()
{
	...
	self.primaryTurretAnim = %aaGunner_aim;
...
turretInit( owner )
{
	...
	self setTurretCanAIDetach( false );
...
turret_main( turret )
{
	...
	// stop threads from _mgturret that also fire turrets
	...
	for ( ;; )
	{
		if ( turret.doFiring )
		{
			...
			self waitTimeOrUntilTurretStateChange( safe_float_range( turret_burst, turret_burst + turret_burst_range ), turret );
			turret notify( "turretstatechange" );

			if ( turret.doFiring )
			{
				thread DoAim( turret );
				wait( safe_float_range( turret_delay, turret_delay + turret_delay_range ) );
			}
		}
		else
		{
...
safe_float_range( min_float, max_float )
{
	...
	{
		return RandomFloatRange( min_float, max_float );
	}
}
...
TurretDoShoot( turret )
{
	...
	for ( ;; )
	{
		if ( !IsDefined( turret.scripted_fire_disabled ) || !turret.scripted_fire_disabled )
		{
			turret turret_fire_alternate( "tag_flash1", "tag_flash2" );
			
			if ( isDefined( turret.barrel_alternate_delay ) )
			{
				wait turret.barrel_alternate_delay;
			}
			else
			{
				wait 0.05;
...
turret_fire_alternate( tag_flash1, tag_flash2 )
{
	if ( !IsDefined( self.turret_fire_number ) )
	{
		self.turret_fire_number = 0;
	}
	
	tag_flash = undefined;
	if ( self.turret_fire_number == 0 )
	{
		tag_flash = tag_flash1;
	}
	else
	{
		tag_flash = tag_flash2;
	}
	...
	MagicBullet( "aa_turret_magicbullet_old", tag_flash_origin, tag_flash_origin + fire_dir );
	
	self.turret_fire_number = ( self.turret_fire_number + 1 ) % 2;
}
